...
class Subject(object):
    ...
    def is_authenticated(self):
        if self.validate_access_token() or self.refresh_token():
            return True
        self.clear()
        return False
    ...
    def redirect_to_authenticate(state=None):
        builder = Subject.create_oauth2_builder(OAuth2Utils.SEGMENT_AUTHORIZE)
    ...
    def authenticate(self, auth_code):
        builder = Subject.create_oauth2_builder(OAuth2Utils.SEGMENT_ACCESS_TOKEN)
        ...
        if builder.validate():
            url = builder.build()
            r = requests.post(url=url, headers=self.create_headers()).json()
    ...
    def logout(self):
        ...
        if refresh_token:
            builder = Subject.create_oauth2_builder(OAuth2Utils.SEGMENT_LOG_OFF)
    ...
    def validate_access_token(self):
        if self.exists(Subject.SESSION_ACCESS_TOKEN) and self.exists(Subject.SESSION_OPEN_ID):
            ...
            if builder.validate():
                url = builder.build()
                r = requests.get(url=url, headers=self.create_headers()).json()
                return ResponseResult.is_ok(r.get(ResponseEntity.RET_ERROR_CODE))
        return False
    ...
    def refresh_token(self):
        ...
        refresh_token = self.get_session_by_key(Subject.SESSION_REFRESH_TOKEN)
        builder = self.create_oauth2_builder(OAuth2Utils.SEGMENT_REFRESH_TOKEN)
    ...
    def get_user_info(self, force_update=False):
        ...
        builder = self.create_oauth2_builder(OAuth2Utils.SEGMENT_GET_USER_INFO)
    ...
    def has_permission(self, permission_name):
        if not self.exists(Subject.SESSION_PERMISSIONS):
            return False

        permission_dict = self.get_session_by_key(Subject.SESSION_PERMISSIONS)
        resource, operation = AuthorizationService.parse_permission(permission_name)
        if resource and operation:
            operations = permission_dict.get(resource)
